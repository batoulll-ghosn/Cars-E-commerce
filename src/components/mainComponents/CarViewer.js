...
const CarViewer = () => {
  ...
  const handleAddToCart = () => {
    if (!localStorage.getItem("ids")) {
      ...
      navigate("/cart");
    } else if (localStorage.getItem("ids").split(",").length >= 2) {
      toast.error(`Can't add more than 2 cars`);
      return;
    } else {
  ...
  return (
    <div>
      <div className="viewer-nav">
        <Link className="viewer-link" to="/cars">
      ...
      <div>
        <div key={cars._id}>
          <h2
            className="viewer-car-price"
            style={{ textDecoration: "line-through" }}
          >
            ${cars.sellingPrice}
          </h2>
          <h2 className="viewer-car-price">
            ${cars.sellingPrice - cars.sellingPrice * (cars.discount / 100)}
          </h2>
      ...
      <div className="viewer-all-infos">
        <div className="viewer-details">
          <div className="viewer-flex">
            ...
            <div id="colorDisplay" className="viewer-circle-color">
              .
            </div>
          </div>
          <p className="viewer-description">{cars.description}</p>
          <button className="viewer-add-to-cart" onClick={handleAddToCart}>
            Add to cart
          </button>
        </div>
        <div>
        ...
        <div className="viewer-more-details">
          ...
          <p className="viewer-description">Company: {cars.company}</p>
          <p className="viewer-description">Type: {cars.type}</p>
          <p className="viewer-description">Date or release: {cars.DOR}</p>
        </div>
      </div>
    </div>
  );
